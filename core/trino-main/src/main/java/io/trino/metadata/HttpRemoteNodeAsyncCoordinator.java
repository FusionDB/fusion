    ...
    public synchronized void asyncRefresh()
    {
        ...
        if (sinceUpdate.toMillis() > 1_000 && future.get() == null) {
            ...
            Request request = preparePost()
                    ...
                    .build();
            JsonCodec<Boolean> CODEC = jsonCodec(Boolean.class);
            HttpResponseFuture<JsonResponse<Boolean>> responseFuture = httpClient.executeAsync(request, createFullJsonResponseHandler(CODEC));
                ...
                public void onSuccess(@Nullable JsonResponse<Boolean> result)
                {
                    ...
                    if (result != null) {
                        ...
                        if (result.getStatusCode() != OK.code()) {
                            log.warn("Error async coordinator info to worker node from %s returned status %d", coordinatorInfoUri, result.getStatusCode());
                            return;
                        }
                    }
                }
