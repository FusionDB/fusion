...
public class QueryExecutor {
    ...
    private static boolean executeCommand(QueryRunner queryRunner, String query, ClientConfig.OutputFormat outputFormat, boolean ignoreErrors) {
        ...
        for (StatementSplitter.Statement split : splitter.getCompleteStatements()) {
            if (!isEmptyStatement(split.statement())) {
                if (!process(queryRunner, split.statement(), outputFormat, getTerminal(),() -> {
                }, false)) {
                    if (!ignoreErrors) {
                        return false;
                    }
    ...
    private static boolean process(QueryRunner queryRunner, String sql, ClientConfig.OutputFormat outputFormat, Terminal terminal,  Runnable schemaChanged, boolean interactive) {
        String finalSql;
        try {
        ...
        try (Query query = queryRunner.startQuery(finalSql)) {
            boolean success = query.renderOutput(context, terminal, System.out, outputFormat, interactive);
